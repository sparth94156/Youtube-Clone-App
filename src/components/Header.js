...
const Header = () => {
    ...
    const cachedInfo = useSelector(store => store.search)

    const handleshowMenu = () => {
        dispatch(showMenu())
    }

    // useEffect hook will render the component when the searchQuery state changes
    ...
    useEffect(() => {
        ...
        return () => {
            // when another api call was made even before 200ms the previous component unmounts or destroys & triggers the reconciliation & it returns the cleanup function to clear things up.
            clearTimeout(timer)
        }
    }, [searchQuery])
                ...
                <div className='w-[50%] flex justify-center items-center gap-x-2'>
                    <div className='flex gap-x-0 '>
                        <input type='text' placeholder='Search'
                            className='px-3 py-2 rounded-s-3xl outline-none w-[500px] border border-gray-400 border-r-0'
                            value={searchQuery}
                            onChange={(e) => setsearchQuery(e.target.value)}
                            onFocus={() => setshowSuggestion(true)}
                            onBlur={() => setshowSuggestion(false)}
                        />
                        <button className='bg-gray-200 px-5 rounded-r-3xl border border-gray-400 border-l-0'>
                            <i class="fa-solid fa-magnifying-glass"></i>
                        </button>
                    </div>
                    <div>
                        <button className='px-3 py-2 rounded-full bg-gray-200 hover:bg-gray-300'>
                            <i class="fa-solid fa-microphone fa-lg "></i>
                        </button>
                    </div>
                </div>
                <div className='flex w-[20%] justify-end items-center gap-x-3'>
                    <div className=''>
                        <div className='p-[6px] rounded-full hover:bg-gray-200 cursor-pointer'>
                            <svg xmlns="http://www.w3.org/2000/svg" width="1.8em" height="1.8em" viewBox="0 0 24 24"><path fill="currentColor" d="M17 10.5V7a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-3.5l4 4v-11zM14 13h-3v3H9v-3H6v-2h3V8h2v3h3z" /></svg>
                        </div>
                    </div>
                    <div className='rounded-full p-2 hover:bg-gray-200 cursor-pointer'>
                        <i class="fa-solid fa-bell fa-xl"></i>
                    </div>
                    <div className=' px-3 py-1 rounded-full cursor-pointer mr-2 w-14 h-10'>
                        <img src='https://yt3.ggpht.com/yti/ANjgQV--BLdBZWMPCaB5-8y9bx_OU1sQ_ODfZMqWNK4H1jo=s88-c-k-c0x00ffffff-no-rj'
                            alt='profile'
                            className='w-[100%] h-[100%] rounded-full flex items-center justify-center hover:opacity-90' />
                    </div>
                </div>
            </div>
            {
